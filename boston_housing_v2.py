...
# 答: 将数据集分割成训练数据和测试数据可以更好地测试模型，可以让我们作出模型后用测试集进行验证，评估模型对新数据的匹配程度，比如是否存在欠拟合和过拟合问题等,可以用于最终的模型评估
...
# 答: 网格搜索法是通过遍历的方法，将给定的参数列表和参数值(给出的值要尽可能的包含潜在的最优参数值)，通过将估计函数的参数通过交叉验证的方法进行优化来得到最优的学习算法。 
# 即，将各个参数可能的取值进行排列组合，列出所有可能的组合结果生成“网格”。根据研究的需要,返回所需,例如当前项目中使用的是 DecisiontreeRegressor返回是项目中使用的是 DecisiontreeRegressor的最大深度,并且可以使用交叉验证对表现进行评估。通过指定的评价函数返回当前给定参数值下的最优.
...
# 答: 
# - 什么是K折交叉验证法（k-fold cross-validation）？
# k折交叉验证法就是将训练集的所有数据平均分成k个子集，其中选取1个子集的数据作为验证集，其他K-1个子集的数据作为训练集来对算法进行训练，如此交叉验证重复k次，
# 
# - [GridSearchCV](http://scikit-learn.org/stable/modules/generated/sklearn.model_selection.GridSearchCV.html) 是如何结合交叉验证来完成对最佳参数组合的选择的？ 
# 
# 每次选择一个子集作为测试集，直到K个部分都做过验证集（即进行了K次单独的训练），然后将K次实验的测试结果取平均值。 这种方法的好处在于，所有的样本都被验证了一次。 
# 
# - [GridSearchCV](http://scikit-learn.org/stable/modules/generated/sklearn.model_selection.GridSearchCV.html) 中的`'cv_results_'`属性能告诉我们什么？
...
# 
# 其中包括 :param_kernel,param_gamma,param_degree,split0_test_score,split1_test_score,mean_test_score,std_test_score,rank_test_score,split0_train_score,split1_train_score,mean_train_score,std_train_score,mean_fit_time,std_fit_time,mean_score_time,std_score_time
# 
# 关键的 key “params”用于存储所有参数候选项的参数设置列表。
# 
# mean_fit_time、std_fit_time、mean_score_time和std_score_time都在几秒钟内完成。
# 
# 
# 网格搜索算法用于系统地遍历多种参数的组合，通过交叉验证来确定最佳效果参数。如果网格搜索时不使用交叉验证则可能使得模型泛化能力无法达到最优。 ‘cv_results_’能够输出一个dict，其中包括相关参数的名称以及对应的值。
# 交叉验证算法与其它的评分方法最重要的区别在于它进行了多次验证，并取了多次评分的平均值,这样做有利于降低样本研究的操作失误,如样本划分比例的不合理等,以减小误差.
# 
...
# 
# 从参数 'RM'来看,RM高,则房价应该高,同时 LSTAT,最低,'PTRATIO'最高,综合考虑以上三个条件,Client 3’s home: $903,393.75,在这三套房中,价格最高符合预期
# 从参数 'RM'来看,RM最低, LSTAT最高,并且'PTRATIO'最高,的Client 2’s home的价格在三套房屋中,总价最低,符合预期;
# 从参数 'RM' 'LSTAT' 以及 'PTRATIO'均处于中间位置的,因此价格中等,符合预期;
...
# 回答:采用不同的训练和测试集执行 fit_model 函数10次，从结果上来看模型足够健壮。
